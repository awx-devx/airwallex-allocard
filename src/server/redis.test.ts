...
describe('redisKeys', () => {
  it('matches ARCHITECTURE §10 conventions', () => {
    ...
    expect(redisKeys.awToken(null)).toBe('aw:token')
    expect(redisKeys.awToken('acct_1')).toBe('aw:token:acct_1')
    expect(redisKeys.awConfig()).toBe('aw:config')
    expect(redisKeys.awConfig('acct_1')).toBe('aw:config:acct_1')
    expect(redisKeys.cardLimits('c1')).toBe('card:limits:c1')

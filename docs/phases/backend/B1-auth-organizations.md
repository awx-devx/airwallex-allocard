...
Auth.js with a Mongoose adapter, credentials provider (argon2 or bcrypt) plus one OAuth provider. Session strategy: JWT, carrying `userId`, `orgId`, `orgRole`, and `onboarded`.
